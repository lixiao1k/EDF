        ...
        if (OSIntNesting == 0) {                           /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    fprintf(stderr, "%u\t%-10s%4u%4u\n", OSTime/100, "Preempt", OSPrioCur, OSTCBHighRdy->OSTCBPrio);
#endif
...
static  void  OS_InitTaskStat (void)
{
    ...
    (void)OSTaskCreateExt(OS_TaskStat,
                          ...
                          (void *)args,//Modification                                    /* No TCB extension               */
    ...
    if (OSIntNesting == 0) {                           /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
				...
				//MY Modification
                fprintf(stderr, "%u\t%-10s%4u%4u\n", OSTime/100, "Complete", OSPrioCur, OSTCBHighRdy->OSTCBPrio);
...
*********************************************************************************************************
*/
//visit TCB list linearly
static  void  OS_SchedNew (void)
{
	//MY Modification
    INT8U highestPriority = OS_LOWEST_PRIO;
...
INT8U  OS_TCBInit (INT8U prio, OS_STK *ptos, OS_STK *pbos, INT16U id, INT32U stk_size, void *pext, INT16U opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
		...
		if (p != 0) {
			ptcb->deadline = p[DEADLINE];
			ptcb->compTime = p[COMP_TIME];
			ptcb->period = p[DEADLINE];
			ptcb->fullCompTime = p[COMP_TIME];
			ptcb->OSTCBDly = p[TIMEOUT_WAITING_TIME];
		}
#else
